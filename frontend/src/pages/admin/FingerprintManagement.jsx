    ...
    return (
      <div className="flex gap-3 text-[10px]">
        <div className="flex-1 bg-red-50 rounded-lg p-2.5 text-center border border-red-100 opacity-50">
          ...
          <span className="text-red-500 font-bold uppercase">{t('fingerprint.stats.users')}</span>
        </div>
        <div className="flex-1 bg-red-50 rounded-lg p-2.5 text-center border border-red-100 opacity-50">
          <span className="block font-bold text-red-800 text-sm">—</span>
          <span className="text-red-500 font-bold uppercase">{t('fingerprint.stats.logs')}</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex gap-3 text-[10px]">
      <div className="flex-1 bg-blue-50/50 rounded-lg p-2.5 text-center border border-blue-100">
        ...
        <span className="text-blue-600 font-bold uppercase tracking-wider">{t('fingerprint.stats.users')}</span>
      </div>
      <div className="flex-1 bg-emerald-50/50 rounded-lg p-2.5 text-center border border-emerald-100">
        ...
        <span className="text-emerald-600 font-bold uppercase tracking-wider">{t('fingerprint.stats.logs')}</span>
      </div>
    </div>
  );
};
...
const FingerprintManagement = () => {
  ...
  const handlePushToDevice = () => {
    if (!pushDeviceId || selectedEmployees.size === 0) return alert(t('fingerprint.push.footer.alertSelect'));
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 text-slate-500 mb-1">
            ...
            <span className="text-[10px] font-bold uppercase tracking-wider">{t('fingerprint.titleDesc')}</span>
          </div>
          <h1 className="text-2xl font-bold text-slate-800">{t('fingerprint.title')}</h1>
        </div>
        <div className="flex gap-2">
          {['devices', 'push'].map(tab => (
            <button key={tab} onClick={() => { setActiveTab(tab); setSelectedDeviceId(null); }}
              ...
              {tab === 'devices' ? t('fingerprint.tabs.dashboard') : t('fingerprint.tabs.push')}
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {devicesLoading ? (
            ...
            <div className="col-span-full text-center py-20 border-2 border-dashed rounded-2xl">
              ...
              <p className="text-sm text-slate-500">{t('fingerprint.dashboard.noDevices')}</p>
            </div>
          ) : devices.map(dev => (
            <div key={dev.id} className="group bg-white border border-slate-200 rounded-2xl p-6 hover:shadow-lg hover:border-blue-300 transition-all duration-300 cursor-pointer"
              ...
              <div className="mt-4 text-center">
                <span className="text-xs text-blue-600 font-bold group-hover:underline">{t('fingerprint.dashboard.clickDetail')}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {activeTab === 'detail' && (
        <div className="space-y-6">
          <button onClick={() => setActiveTab('devices')} className="flex items-center gap-2 text-sm text-slate-600 hover:text-blue-600 transition-colors font-semibold">
            <ChevronLeft className="w-4 h-4" /> {t('fingerprint.detail.back')}
          </button>

          {detailLoading ? (
            <div className="flex justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-blue-500" /></div>
          ) : !detail ? (
            <div className="text-center py-20 text-red-500">{t('fingerprint.detail.failed')}</div>
          ) : (
            <>
              {/* Device Info Card */}
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <div className="flex flex-wrap gap-3 mt-6">
                  <button onClick={() => refetchDetail()} className="px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-bold flex items-center gap-2 hover:bg-slate-50 transition-all">
                    <RefreshCw className="w-4 h-4" /> {t('fingerprint.detail.refresh')}
                  </button>
                  ...
                  <button onClick={() => setEnrollState({ open: true, step: 1, employeeId: '', fingerId: '0', status: 'idle', message: '', uid: null, acNo: null })}
                    ...
                    <Fingerprint className="w-4 h-4" /> {t('fingerprint.detail.register')}
                  </button>
                  <button 
                    onClick={() => {
                      if (confirm(t('fingerprint.detail.clearLogsConfirm', { name: detail.device.name }))) {
                  ...
                  >
                    ...
                    {t('fingerprint.detail.clearLogs')}
                  </button>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="p-5 border-b border-slate-100 flex items-center justify-between">
                  <h3 className="font-bold text-slate-800 flex items-center gap-2"><Users className="w-5 h-5 text-blue-600" /> {t('fingerprint.detail.tableTitle', { count: detail.machineUsers?.length || 0 })}</h3>
                </div>
                <div className="overflow-x-auto max-h-[500px] overflow-y-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 sticky top-0 z-10">
                      <tr>
                        <th className="p-3 text-left font-bold text-slate-500 text-xs uppercase">{t('fingerprint.detail.table.uid')}</th>
                        <th className="p-3 text-left font-bold text-slate-500 text-xs uppercase">{t('fingerprint.detail.table.pin')}</th>
                        <th className="p-3 text-left font-bold text-slate-500 text-xs uppercase">{t('fingerprint.detail.table.machineName')}</th>
                        <th className="p-3 text-left font-bold text-slate-500 text-xs uppercase">{t('fingerprint.detail.table.linkStatus')}</th>
                        <th className="p-3 text-left font-bold text-slate-500 text-xs uppercase">{t('fingerprint.detail.table.employee')}</th>
                        <th className="p-3 text-center font-bold text-slate-500 text-xs uppercase">{t('fingerprint.detail.table.actions')}</th>
                      </tr>
                    </thead>
                    <tbody>
                          ...
                          <td className="p-3">
                            {mu.linked ? (
                              <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded-lg text-xs font-bold border border-emerald-100">
                                <CheckCircle2 className="w-3.5 h-3.5" /> {t('fingerprint.detail.table.linked')}
                              </span>
                            ) : (
                              <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-red-50 text-red-600 rounded-lg text-xs font-bold border border-red-100">
                                <XCircle className="w-3.5 h-3.5" /> {t('fingerprint.detail.table.unlinked')}
                              </span>
                            )}
                          </td>
                          <td className="p-3">
                            {mu.employee ? (
                              ...
                              >
                                {t('fingerprint.detail.table.linkBtn')}
                              </button>
                            )}
                          </td>
                          <td className="p-3 text-center">
                            <button 
                              onClick={(e) => { e.stopPropagation(); if (confirm(t('fingerprint.detail.table.deleteConfirm', { uid: mu.uid, name: mu.name }))) deleteUserMutation.mutate({ deviceId: selectedDeviceId, uid: mu.uid }); }}
                              className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all"
                              title={t('fingerprint.detail.table.deleteTooltip')}>
                      ...
                      {(!detail.machineUsers || detail.machineUsers.length === 0) && (
                        <tr><td colSpan="6" className="p-8 text-center text-slate-400">
                          {detail.deviceInfo ? t('fingerprint.detail.table.noUsers') : t('fingerprint.detail.table.offlineNotice')}
                        </td></tr>
                      )}
                    </tbody>
                  </table>
      ...
      {activeTab === 'push' && (
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2"><Upload className="w-5 h-5 text-blue-600" /> {t('fingerprint.push.title')}</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase block mb-2">{t('fingerprint.push.selectMachine')}</label>
                <select value={pushDeviceId} onChange={e => setPushDeviceId(e.target.value)}
                  ...
                  <option value="">{t('fingerprint.push.selectMachinePlaceholder')}</option>
                  {deviceList.map(d => <option key={d.id} value={d.id}>{d.name} ({d.ipAddress})</option>)}
                </select>
              </div>
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase block mb-2">{t('fingerprint.push.searchEmployee')}</label>
                <div className="relative">
                  <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                  <input type="text" placeholder={t('fingerprint.push.searchPlaceholder')}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <td className="p-3 text-center">
                          {emp._count?.fingerTemplates > 0 ? (
                            <span className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-50 text-emerald-700 rounded-lg text-xs font-bold border border-emerald-100">
                              {t('fingerprint.push.table.fingerCount', { count: emp._count.fingerTemplates })}
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 px-2 py-1 bg-amber-50 text-amber-600 rounded-lg text-xs font-bold border border-amber-100">
                              {t('fingerprint.push.table.notRegistered')}
                            </span>
                          )}
                        </td>
                        <td className="p-3 text-center">
                          <span className="text-xs font-bold text-slate-500">{t('fingerprint.push.table.machineCount', { count: emp._count?.deviceUsers || 0 })}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <div className="p-4 bg-blue-50 border-t border-blue-100 flex flex-col sm:flex-row justify-between items-center gap-4">
              <div className="flex items-center gap-3">
                <span className="text-blue-700 font-bold text-sm">{t('fingerprint.push.footer.selected', { count: selectedEmployees.size })}</span>
                <div className="flex items-center gap-1.5 text-xs text-blue-500">
                  <Info className="w-3.5 h-3.5" />
                  <span>{t('fingerprint.push.footer.notice')}</span>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-slate-800">{t('fingerprint.linkModal.title')}</h3>
              <button onClick={() => { setLinkingUser(null); setSelectedEmpForLink(''); }} className="text-slate-400 hover:text-slate-600">✕</button>
            </div>
            
            <p className="text-xs text-slate-500 mb-4 leading-relaxed">
              {t('fingerprint.linkModal.description', { pin: linkingUser.userId, name: linkingUser.name || 'No Name' })}
            </p>
            
            <div className="space-y-4">
              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">{t('fingerprint.linkModal.selectEmployee')}</label>
                <select 
                ...
                >
                  <option value="">{t('fingerprint.linkModal.selectEmployeePlaceholder')}</option>
                  {employees
      ...
      {enrollState.open && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2"><Fingerprint className="w-5 h-5 text-blue-600" /> {t('fingerprint.enrollModal.title')}</h3>
            ...
            {enrollState.step === 1 && (
              <div className="space-y-4">
                <p className="text-xs text-slate-500">{t('fingerprint.enrollModal.description')}</p>
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">{t('fingerprint.enrollModal.employee')}</label>
                  <select 
                  ...
                  >
                    <option value="">{t('fingerprint.enrollModal.employeePlaceholder')}</option>
                    {employees.map(emp => (
                      <option key={emp.id} value={emp.id}>{emp.name} ({emp.employeeCode})</option>
                    ))}
                  </select>
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">{t('fingerprint.enrollModal.finger')}</label>
                  <select 
                ...
                <div className="pt-2 flex justify-end gap-3">
                  ...
                  >
                    ...
                    {t('fingerprint.enrollModal.startBtn')}
                  </button>
                </div>
              </div>
            )}
              ...
              <div className="space-y-5 text-center py-4">
                {enrollState.status === 'enrolling' && (
                  <>
                    ...
                    <h4 className="font-bold text-slate-800">{t('fingerprint.enrollModal.readyTitle')}</h4>
                    <p className="text-xs text-slate-500 px-4 leading-relaxed">
                      Sensor pada mesin sidik jari sekarang menyala. {t('fingerprint.enrollModal.readyDesc', { finger: FINGER_MAP.find(f => f.id === enrollState.fingerId)?.name })}
                    </p>
                ...
                {enrollState.status === 'verifying' && (
                  <>
                    ...
                    <h4 className="font-bold text-slate-800">{t('fingerprint.enrollModal.verifyingTitle')}</h4>
                    <p className="text-xs text-slate-500">{t('fingerprint.enrollModal.verifyingDesc')}</p>
                  </>
                )}
                ...
                {enrollState.status === 'error' && (
                  <>
                    ...
                    <h4 className="font-bold text-red-600">{t('fingerprint.enrollModal.failedTitle')}</h4>
                    ...
                    >
                      {t('fingerprint.enrollModal.failedRetry')}
                    </button>
                  </>
                )}
                ...
                <div className="pt-4 border-t border-slate-100 flex justify-between gap-3">
                  <button 
                    ...
                    className="px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-lg text-xs font-bold text-slate-600 transition-all"
                  >
                    {t('fingerprint.enrollModal.back')}
                  </button>
                  <button 
                    onClick={() => {
                  ...
                  >
                    ...
                    {t('fingerprint.enrollModal.verifySave')}
                  </button>
                </div>
              </div>
            )}
            ...
            {enrollState.step === 3 && (
              <div className="space-y-4 text-center py-4">
                ...
                <h4 className="font-bold text-emerald-700">{t('fingerprint.enrollModal.successTitle')}</h4>
                <p className="text-xs text-slate-500 px-4 leading-relaxed">
                  {t('fingerprint.enrollModal.successDesc')}
                </p>
                ...
                <div className="pt-4 flex justify-center">
                  ...
                  >
                    {t('fingerprint.enrollModal.finish')}
                  </button>
                </div>
              </div>
            )}
