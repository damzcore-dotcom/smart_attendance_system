...
const DailyWorkerAttendance = () => {
  ...
  const { data: employeesData, isLoading: empLoading } = useQuery({
    queryKey: ['employees-for-bhl', deptFilter],
    // Based on prisma schema: EmployeeSalary has employmentType
    // For now we get all and filter locally, or pass it if backend supports.
        ...
        <div className="flex items-center gap-3 w-full lg:w-1/3">
           ...
           <div className="flex-1">
             ...
             <input 
               ...
               className="w-full text-sm font-semibold border-b border-slate-200 pb-1 focus:outline-none focus:border-emerald-600 bg-transparent"
             />
           </div>
        </div>
