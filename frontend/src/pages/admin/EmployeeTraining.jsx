...
import { 
  GraduationCap, Search, Filter, ChevronRight, Loader2, Calendar, AlertCircle, Clock, Users, ArrowUpRight
} from 'lucide-react';

const EmployeeTraining = ({ isReadOnly = false }) => {
  ...
  // Filter only training employees who are active (not Terminated)
  const trainingEmployees = allEmployees.filter(e => {
    const empStatus = (e.employmentStatus || '').toUpperCase();
    const isTraining = empStatus.includes('TRAINING');
    const isTerminated = e.status === 'Terminated' || e.status === 'TERMINATED';
    return isTraining && !isTerminated;
  });

  const now = new Date();
  now.setHours(0, 0, 0, 0);

  // Compute days left and alert level for each training employee
  const employeesWithTrainingInfo = trainingEmployees.map(emp => {
    let daysLeft = null;
  ...
  const getAlertStatusLabel = (level, daysLeft) => {
    if (level === 'expired') return t('training.statusExpired');
    if (daysLeft !== null) {
      if (daysLeft <= 7) return `${t('training.statusCritical')} (${daysLeft} ${t('training.days')})`;
      if (daysLeft <= 30) return `${t('training.statusApproachingEnd')} (${daysLeft} ${t('training.days')})`;
      return `${t('training.statusActive')} (${daysLeft} ${t('training.days')})`;
    }
    return t('training.statusActive');
  };
      ...
      <div className="print:hidden space-y-8">
        ...
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 px-1">
          <div className="space-y-1">
            <div className="flex items-center gap-3 text-slate-500">
              ...
              <span className="text-[10px] font-bold text-blue-600 uppercase tracking-wider">{t('training.subtitle')}</span>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 px-1">
          ...
          >
            <div className="space-y-1.5">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t('training.totalTrainingEmployees')}</span>
              <div className="text-2xl font-bold text-slate-800 tracking-tight">
                {totalTraining} <span className="text-xs text-slate-500 font-medium">{t('employees.stats.people')}</span>
              </div>
              <p className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">
                {t('training.trainingStatus')}
              </p>
            </div>
          ...
          >
            <div className="space-y-1.5">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t('training.activeSecure')}</span>
          ...
          >
            <div className="space-y-1.5">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t('training.expiringSoon')}</span>
          ...
          >
            <div className="space-y-1.5">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t('training.expired')}</span>
        ...
        <div className="bg-white p-6 border border-slate-200 rounded-2xl shadow-sm mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider ml-1">{t('training.alertStatus')}</label>
              <div className="relative">
                <select 
                  value={statusFilter} 
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            ...
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                  ...
                  <th className="px-6 py-4">{t('training.trainingDuration')}</th>
                  <th className="px-6 py-4 text-center">{t('training.startTraining')}</th>
                  <th className="px-6 py-4 text-center">{t('training.endTraining')}</th>
                  <th className="px-6 py-4 text-center">{t('training.daysLeft')}</th>
                  <th className="px-6 py-4">{t('training.alertStatus')}</th>
                </tr>
              </thead>
          ...
          <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between">
            <p className="text-xs text-slate-500 font-medium">
              {t('training.showing')} <span className="font-bold text-slate-800">{paginatedEmployees.length}</span> {t('training.of')} <span className="font-bold text-slate-800">{totalItems}</span> {t('training.data')} | {t('training.page')} <span className="font-bold text-slate-800">{page}</span> {t('training.of')} <span className="font-bold text-slate-800">{totalPages}</span>
            </p>
