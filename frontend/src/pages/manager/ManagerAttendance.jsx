...
const STATUS_ICONS = {
  ...
  'CUTI': AlertCircle,
  'HOLIDAY': AlertCircle,
  // Indonesian label variants (sent by backend)
  'Hadir': CheckCircle2,
  'Terlambat': Clock,
  'Mangkir': AlertCircle,
  'Alpha': XCircle,
...
const ManagerAttendance = () => {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['manager-attendance', appliedFilters],
  ...
  const { data: fullDataForRekap, isLoading: isRekapLoading } = useQuery({
    queryKey: ['manager-attendance-full-rekap', appliedFilters],
    queryFn: () => managerAPI.getAttendance({ ...appliedFilters, limit: 99999, page: 1 }),
  ...
  const handleExport = () => {
    try {
      ...
      if (activeViewTab === 'REKAPITULASI') {
        ...
        XLSX.writeFile(wb, `Rekap_Absensi_Per_Dept_Manager_${new Date().toISOString().split('T')[0]}.xlsx`);
        return;
      }
      ...
      const filePrefix = isIndo ? 'Laporan_Absensi_Manager' : isKo ? '매니저_근태_보고서' : isZh ? '经理考勤报告' : 'Manager_Attendance_Report';
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 tracking-tight">{t('attendancePage.titleManager')}</h1>
          <p className="text-sm text-slate-500 mt-1 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-500" />
            {t('attendancePage.descManager')}
          </p>
        </div>
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 text-rose-600 font-bold text-xs mt-3 bg-rose-50 px-3 py-1.5 rounded-full w-fit">
                      ...
                      {t('attendancePage.includesPenalty', { penalty: summary.mangkirPenalty || 30 })}
                    </div>
                  </div>
                </div>
          ...
          <table className="w-full text-left whitespace-nowrap border-separate border-spacing-0">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-200">
                ...
                <th className="px-6 py-4 border-b border-slate-200 text-center text-[10px] font-bold text-slate-500 uppercase tracking-widest">{t('attendancePage.checkIn')}</th>
                <th className="px-6 py-4 border-b border-slate-200 text-center text-[10px] font-bold text-slate-500 uppercase tracking-widest">{t('attendancePage.checkOut')}</th>
                <th className="px-6 py-4 border-b border-slate-200 text-center text-[10px] font-bold text-slate-500 uppercase tracking-widest">{t('attendancePage.late')}</th>
              ...
              {isLoading ? (
                ...
                return (
                  <tr key={`${r.id}-${idx}`} className="group hover:bg-blue-50/40 transition-colors duration-200">
                    <td className="px-6 py-3 sticky left-0 bg-white group-hover:bg-blue-50/50 z-10 border-r border-slate-50 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.01)]">
                      <div className="flex items-center gap-3 min-w-[200px]">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-[10px] shrink-0 border border-white shadow-sm ${avatarColor}`}>
                          {initials}
                        </div>
                        <div>
                          <p 
                    ...
                    <td className="px-6 py-3">
                      <span className="text-xs font-bold text-slate-700 block">{r.dept || '-'}</span>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">{r.section || '-'}</p>
                    </td>
                    <td className="px-6 py-3 text-center">
                      <span className="px-2.5 py-1 rounded-md bg-slate-100 border border-slate-200 text-[10px] font-black text-slate-700">
                    ...
                    <td className="px-6 py-3 text-center text-xs font-bold text-slate-800">{formatTime(r.checkIn)}</td>
                    <td className="px-6 py-3 text-center text-xs font-bold text-slate-800">{formatTime(r.checkOut)}</td>
                    <td className="px-6 py-3 text-center">
                       {(r.status?.toUpperCase() === 'LATE' || r.status === 'Terlambat') && r.lateMinutes > 0 ? (
            ...
            <table className="w-full text-left border-separate border-spacing-0">
              <thead className="sticky top-0 z-30 bg-slate-50 border-b border-slate-200">
                <tr className="text-slate-500 text-[10px] font-bold uppercase tracking-wider bg-slate-50">
                  <th className="px-6 py-3 text-center">{isIndo ? 'No' : isKo ? '번호' : isZh ? '序号' : 'No'}</th>
                  <th className="px-6 py-3">{isIndo ? 'Karyawan' : isKo ? '사원' : isZh ? '员工' : 'Employee'}</th>
                  <th className="px-6 py-3">{isIndo ? 'Dept' : isKo ? '부서' : isZh ? '部门' : 'Dept'}</th>
                  <th className="px-4 py-3">{isIndo ? 'Bagian' : isKo ? '파트' : isZh ? '班组' : 'Section'}</th>
                  <th className="px-4 py-3 text-center">{isIndo ? 'Total' : isKo ? '총 일정' : isZh ? '总计' : 'Total'}</th>
                  <th className="px-4 py-3 text-center">{isIndo ? 'Hadir' : isKo ? '출석' : isZh ? '出勤' : 'Present'}</th>
                  <th className="px-4 py-3 text-center">{isIndo ? 'Telat' : isKo ? '지각' : isZh ? '迟到' : 'Late'}</th>
                  <th className="px-4 py-3 text-center">{isIndo ? 'P. Cepat' : isKo ? '조퇴' : isZh ? '早退' : 'Early'}</th>
                  <th className="px-4 py-3 text-center">{isIndo ? 'Mangkir' : isKo ? '무단결근' : isZh ? '旷工' : 'Unexcused'}</th>
                  <th className="px-4 py-3 text-center">{isIndo ? 'Alpa' : isKo ? '결근' : isZh ? '缺勤' : 'Absent'}</th>
                  <th className="px-4 py-3 text-center">{isIndo ? 'Lainnya' : isKo ? '기타' : isZh ? '其他' : 'Others'}</th>
                  <th className="px-6 py-3 text-center">{isIndo ? 'Rasio %' : isKo ? '출석률 %' : isZh ? '比例 %' : 'Rate %'}</th>
                  <th className="px-6 py-3 text-center">{isIndo ? 'Durasi Telat' : isKo ? '지각 시간' : isZh ? '迟到时长' : 'Late Duration'}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {isRekapLoading ? (
...
const FilterBar = ({ onApply, isLoading, currentSearch }) => {
  ...
  const { data: optionsData } = useQuery({
    ...
    queryFn: () => managerAPI.getAttendanceOptions({ period: filterDate, startDate: customStart, endDate: customEnd, dept: filterDept, search: searchQuery }),
    staleTime: 30000,
  });
